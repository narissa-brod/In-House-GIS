-- Use geometry area as a final fallback for acreage when parcel_acres and size_acres are NULL
...
BEGIN
  ...
  WHERE
    ...
    -- City filter: attribute or spatial boundary match (requires municipal_boundaries to be populated to use spatial path)
    AND (
...
COMMENT ON FUNCTION public.search_parcels IS 'Adds geometry area fallback for acres; retains spatial city fallback and vacant inference.';
