-- Alternative GP filtering strategy that doesn't use ST_Union
-- Instead, checks if parcel intersects with ANY GP zone in the selected categories
-- This is simpler and avoids geometry union issues
...
BEGIN
  ...
  WHERE
    ...
    -- General Plan filter: Check if parcel centroid intersects with ANY GP zone in selected categories
    -- Using EXISTS with ST_Contains is efficient because it uses spatial indexes and stops at first match
    AND (
      gp_zones IS NULL OR
      EXISTS (
        SELECT 1
...
COMMENT ON FUNCTION public.search_parcels IS 'Search parcels with optional GP zone filtering. Uses spatial index to check if parcel centroid falls within any GP zone of the selected categories. Fast and reliable.';

-- Create a partial spatial index on general_plan for faster lookups
CREATE INDEX IF NOT EXISTS idx_general_plan_normalized_category ON public.general_plan(normalized_category);
