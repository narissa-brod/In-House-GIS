-- Refine search_parcels to better handle Vacant logic and city name variants
-- - Vacant selected now means prop_class ILIKE '%VACANT%' OR computed has_bldg=false
-- - City filter matches prefixes (e.g., 'WEST POINT' matches 'WEST POINT CITY')
-- - Case-insensitive class matching using prefixes (e.g., 'RESIDENTIAL', 'COMMERCIAL')
-- Replaces previous definition while keeping the same signature
...
BEGIN
  ...
  WITH base AS (
    SELECT p.*,
           -- Compute a conservative building presence flag
           (
  ...
  WHERE
    ...
    AND (
      classes_u IS NULL
      OR (
           -- Include explicit non-vacant classes (prefix match, case-insensitive)
           EXISTS (
             SELECT 1 FROM unnest(classes_u) c
             WHERE c <> 'VACANT' AND upper(b.prop_class) LIKE c || '%'
           )
           OR (
             -- Include vacant when requested: either class says vacant or computed has_bldg=false
             has_vacant AND (
    ...
    -- City filter: allow prefix matches so 'WEST POINT' matches 'WEST POINT CITY'
    AND (
      norm_cities IS NULL OR EXISTS (
        SELECT 1 FROM unnest(norm_cities) c WHERE upper(trim(b.city)) LIKE c || '%'
      )
    )
...
COMMENT ON FUNCTION public.search_parcels IS 'Refined: better Vacant inference (has_bldg flag) and city prefix matching; acreage fallback via COALESCE.';
